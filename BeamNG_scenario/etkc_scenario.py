...
for car_speed in speed:
    my_dict={'etk_transmission' : ['etk_transmission_6M_sport', 'etk_transmission_8DCT','etk_transmission_6M_diesel'],
     'tire_R_19x10':['tire_R_255_35_19_sport','tire_R_255_35_19_race','tire_F_235_35_19_sport'],
     'brakepad_R':['brakepad_R_sport','brakepad_R','brakepad_R_semi_race']}
    ...
    for x in list(combinations):
        ...
        vehicle1 = Vehicle('ego_vehicle', model='etkc', licence='Main01', color = 'White')
        ...
        config_trans['parts']['etk_transmission'] = x[1]
        set_config_trans = vehicle1.set_part_config(config_trans)

        if (x[1] == 'etk_transmission_6M_sport'):
            trans_mission = 'transmission3'
        elif (x[1] == 'etk_transmission_8DCT'):
        ...
        config_tire['parts']['tire_R_19x10'] = x[2]
        set_config_tire = vehicle1.set_part_config(config_tire)

        if (x[2] == 'tire_R_255_35_19_sport'):
            tire_main = 'tire2'
        elif (x[2] == 'tire_R_255_35_19_race'):
            tire_main = 'tire3'
        else:
            tire_main = 'tire1'
            ...
            if (vehicle_distance1 <= 20 and car_speed_km == 30) or (vehicle_distance1 <= 20 and car_speed_km == 35) or (vehicle_distance1 <= 23 and car_speed_km == 40) :
                ...
                if (ispass == 1 or ispass == 0):
                    ...
                    with open(file_name_csv, 'a') as f:
                        ...
                        if fileEmpty:
                            writer.writeheader()
                        writer.writerow({ 'Tire': tire_main ,'Brake' : brake_main ,'Transmission' : trans_mission, 'Car_model': 'etkc' , 'Speed': car_speed_km, 'Result' : ispass})
                break
            elif (vehicle_distance1 < 25 and car_speed_km == 45) or (vehicle_distance1 <= 25 and car_speed_km == 50) or (vehicle_distance1 <= 25 and car_speed_km == 55) :
                vehicle1.ai_set_mode('stopping')
                print(vehicle_distance1)
                time.sleep(5)

                if ( vehicle_distance1 > 23):
                    ispass = 1
                    print('pass')
                else:
                    ispass = 0
                ...
                if (ispass == 1 or ispass == 0):
                    path_dir = os.getcwd()
                    file_name_csv = (path_dir + '\dataset_etkc_new.csv')
                    v = random.randint(0, 100)
                    #with open(r'C:\Users\hamza\OneDrive\Desktop\Boundary\datset.csv', 'a') as f:
                    with open(file_name_csv, 'a') as f:
                        headers = ['Brake', 'Transmission', 'Tire', 'Speed', 'Car_model', 'Result']
                        ...
                        writer.writerow({ 'Tire': tire_main ,'Brake' : brake_main ,'Transmission' : trans_mission, 'Car_model': 'etkc' , 'Speed': car_speed_km, 'Result' : ispass})
                break
            elif (vehicle_distance1 <= 25 and car_speed_km == 60) or (vehicle_distance1 <= 25 and car_speed_km == 65) or (vehicle_distance1 <= 45 and car_speed_km == 70) :
                vehicle1.ai_set_mode('stopping')
                time.sleep(5)

                if (vehicle_distance1 > 22):
                    ispass = 1
                    print('pass')
                else:
                    ispass = 0
